"""Google Mapsスクレイピングのデバッグスクリプト（Async版）"""
...
async def debug_google_maps(query: str = "渋谷 AGA"):
    """Google Mapsの検索結果を調査"""
    ...
    async with async_playwright() as p:
        ...
        await asyncio.sleep(5)  # ページ読み込み待機
        ...
        # 現在使用しているセレクタ
        results1 = await page.query_selector_all('a[href*="/maps/place/"]')
        print(f'a[href*="/maps/place/"]: {len(results1)} elements')

        # 代替セレクタ
        results2 = await page.query_selector_all('[role="feed"] > div')
        print(f'[role="feed"] > div: {len(results2)} elements')

        results3 = await page.query_selector_all('div[role="article"]')
        print(f'div[role="article"]: {len(results3)} elements')

        results4 = await page.query_selector_all('.Nv2PK')
        print(f'.Nv2PK: {len(results4)} elements')

        results5 = await page.query_selector_all('[data-result-index]')
        ...
        if results1:
            ...
            # aria-label属性を確認
            label = await first.get_attribute("aria-label")
            ...
            # クリック後のh1
            h1_after = await page.query_selector("h1")
            if h1_after:
                print(f"h1 after click: '{await h1_after.inner_text()}'")
            ...
            # ウェブサイトボタン
            website = await page.query_selector('[data-item-id="authority"]')
            if website:
                print(f"Website URL: {await website.get_attribute('href')}")
            else:
                print("Website button not found with [data-item-id='authority']")

                # 代替セレクタ
                website2 = await page.query_selector('a[data-value="ウェブサイト"]')
                if website2:
                    print(f"Website (alt): {await website2.get_attribute('href')}")
                else:
                    print("Website button not found with a[data-value='ウェブサイト']")

            # 住所
            address = await page.query_selector('[data-item-id="address"] .fontBodyMedium')
            if address:
                print(f"Address: {await address.inner_text()}")
            else:
                print("Address not found")

            # 電話番号
            phone = await page.query_selector('[data-item-id^="phone"]')
            if phone:
                print(f"Phone: {await phone.inner_text()}")
            else:
...
if __name__ == "__main__":
    ...
    asyncio.run(debug_google_maps(query))
